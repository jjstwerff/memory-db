...
/**
 * Automatically generated record class for fields
 */

@RecordData(name = "RecField")
public class FieldsArray implements ChangeInterface, Iterable<FieldsArray> {
	...
	/* package private */ FieldsArray(Record parent, int idx) {
	...
	/* package private */ FieldsArray(FieldsArray other, int idx) {
	...
	/* package private */ FieldsArray(Store store, int rec, int idx) {
		this.store = store;
		this.alloc = rec;
		this.idx = idx;
	...
	/* package private */ void setUpRecord(Record record) {
		store.setInt(alloc, 8, record.getRec());
	}

	@Override
	public Record getUpRecord() {
		return new Record(store, store.getInt(alloc, 8));
	}
	...
	/* package private */ FieldsArray add() {
		if (parent.getRec() == 0)
			return this;
		idx = size;
		if (alloc == 0) {
	...
	@Override
	public Iterator<FieldsArray> iterator() {
		return new Iterator<>() {
			int element = -1;
			...
			@Override
			public FieldsArray next() {
				if (alloc == 0 || element > size)
					throw new NoSuchElementException();
				element++;
				return new FieldsArray(FieldsArray.this, element);
			}
		};
	}
	...
	public void parse(Parser parser) {
		...
		if (parser.hasField("field")) {
			parser.getRelation("field", (int recNr) -> {
				Iterator<Field> iterator = getUpRecord().getFieldOnName().iterator();
				Field relRec = iterator.hasNext() ? iterator.next() : null;
