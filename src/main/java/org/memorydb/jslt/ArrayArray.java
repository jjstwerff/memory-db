...
public class ArrayArray implements MemoryRecord, ChangeOperator, Iterable<ArrayArray> {
	...
	@Override
	public ArrayArray copy(int rec) {
		assert store.validate(rec);
		return new ArrayArray(store, rec, -1);
	}
	...
	@Override
	public ArrayArray parseKey(Parser parser) {
		return null;
	}
	...
	public Object java() {
		int field = 0;
		if (idx == -1)
			return field < 1 || field > size ? null : new ArrayArray(parent, field - 1);
		if (field >= 0 && field <= 28)
