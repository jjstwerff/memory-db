...
/**
 * Automatically generated record class for ifTrue
 */

@RecordData(name = "Step")
public class IfTrueArray implements MemoryRecord, ChangeOperator, Iterable<IfTrueArray> {
	...
	/* package private */ IfTrueArray(Operator parent, int idx) {
	...
	/* package private */ IfTrueArray(IfTrueArray other, int idx) {
	...
	/* package private */ IfTrueArray(Store store, int rec, int idx) {
		this.store = store;
		this.alloc = rec;
		this.idx = idx;
		this.parent = up();
	...
	@Override
	public IfTrueArray copy(int newRec) {
		assert store.validate(newRec);
		return new IfTrueArray(store, newRec, -1);
	}
	...
	public IfTrueArray add() {
		...
		size++;
		store.setInt(alloc, 4, size);
		IfTrueArray res = new IfTrueArray(parent, size - 1);
		return res;
	}

	@Override
	public Iterator<IfTrueArray> iterator() {
		return new Iterator<>() {
			int element = -1;
	...
	@Override
	public IfTrueArray index(int idx) {
		return idx < 0 || idx >= size ? null : new IfTrueArray(parent, idx);
	}

	@Override
	public IfTrueArray start() {
		return new IfTrueArray(parent, 0);
	}

	@Override
	public IfTrueArray next() {
		return idx + 1 >= size ? null : new IfTrueArray(parent, idx + 1);
	}
	...
	@Override
	public IfTrueArray copy() {
		return new IfTrueArray(parent, idx);
	}
}
