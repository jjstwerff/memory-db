...
/**
 * Automatically generated record class for sortBlock
 */

public class SortBlockArray implements ChangeOperator, Iterable<SortBlockArray> {
	...
	public SortBlockArray(Operator parent, int idx) {
		this.store = parent.getStore();
		this.parent = parent;
		this.idx = idx;
		if (parent.getRec() != 0) {
			this.alloc = store.getInt(parent.getRec(), parent.operatorPosition() + 9);
			if (alloc != 0) {
				setUpRecord(parent);
	...
	public SortBlockArray(SortBlockArray other, int idx) {
	...
	public SortBlockArray(Store store, int rec, int idx) {
		this.store = store;
		this.alloc = rec;
		this.idx = idx;
	...
	public SortBlockArray add() {
		...
		store.setInt(parent.getRec(), parent.operatorPosition() + 9, alloc);
		size = idx + 1;
	...
	@Override
	public Iterator<SortBlockArray> iterator() {
		return new Iterator<>() {
			int element = -1;
			...
			@Override
			public SortBlockArray next() {
				if (element > size)
					throw new NoSuchElementException();
				element++;
				return new SortBlockArray(SortBlockArray.this, element);
			}
		};
	}
