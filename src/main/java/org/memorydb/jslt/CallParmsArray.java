...
public class CallParmsArray implements MemoryRecord, ChangeOperator, Iterable<CallParmsArray> {
	...
	@Override
	public CallParmsArray copy(int rec) {
		assert store.validate(rec);
		return new CallParmsArray(store, rec, -1);
	}
	...
	@Override
	public CallParmsArray parseKey(Parser parser) {
		return null;
	}
	...
	public Object java() {
		int field = 0;
		if (idx == -1)
			return field < 1 || field > size ? null : new CallParmsArray(parent, field - 1);
		if (field >= 0 && field <= 28)
