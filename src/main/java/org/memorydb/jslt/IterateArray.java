...
/**
 * Automatically generated record class for iterate
 */

@RecordData(name = "SubMatch")
public class IterateArray implements ChangeMatch, Iterable<IterateArray> {
	...
	/* package private */ IterateArray(Match parent, int idx) {
	...
	/* package private */ IterateArray(IterateArray other, int idx) {
	...
	/* package private */ IterateArray(Store store, int rec, int idx) {
		this.store = store;
		this.alloc = rec;
		this.idx = idx;
	...
	/* package private */ IterateArray add() {
		if (parent.getRec() == 0)
			return this;
		idx = size;
		if (alloc == 0) {
	...
	@Override
	public Iterator<IterateArray> iterator() {
		return new Iterator<>() {
			int element = -1;
			...
			@Override
			public IterateArray next() {
				if (alloc == 0 || element > size)
					throw new NoSuchElementException();
				element++;
				return new IterateArray(IterateArray.this, element);
			}
		};
	}
