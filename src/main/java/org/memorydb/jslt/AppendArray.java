...
public class AppendArray implements MemoryRecord, ChangeOperator, Iterable<AppendArray> {
	...
	@Override
	public AppendArray copy(int newRec) {
		assert store.validate(newRec);
		return new AppendArray(store, newRec, -1);
	}
	...
	public AppendArray index(int idx) {
		return idx < 0 || idx >= size ? null : new AppendArray(parent, idx);
	}

	@Override
	public AppendArray start() {
		return new AppendArray(parent, 0);
	}

	@Override
	public AppendArray next() {
		return idx + 1 >= size ? null : new AppendArray(parent, idx + 1);
	}
