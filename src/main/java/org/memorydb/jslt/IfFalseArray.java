...
/**
 * Automatically generated record class for ifFalse
 */

@RecordData(name = "Step")
public class IfFalseArray implements MemoryRecord, ChangeOperator, Iterable<IfFalseArray> {
	...
	/* package private */ IfFalseArray(Operator parent, int idx) {
		this.store = parent.store();
		this.parent = parent;
		this.idx = idx;
		if (parent.rec() != 0) {
			this.alloc = store.getInt(parent.rec(), parent.operatorPosition() + 9);
			if (alloc != 0) {
				up(parent);
	...
	/* package private */ IfFalseArray(IfFalseArray other, int idx) {
	...
	/* package private */ IfFalseArray(Store store, int rec, int idx) {
		this.store = store;
		this.alloc = rec;
		this.idx = idx;
		this.parent = up();
	...
	public IfFalseArray add() {
		...
		size++;
		store.setInt(alloc, 4, size);
		IfFalseArray res = new IfFalseArray(parent, size - 1);
		return res;
	}

	@Override
	public Iterator<IfFalseArray> iterator() {
		return new Iterator<>() {
			int element = -1;
			...
			public IfFalseArray next() {
				...
				element++;
				return new IfFalseArray(IfFalseArray.this, element);
			}
	...
	@Override
	public IfFalseArray parseKey(Parser parser) {
		return null;
	}
	...
	@Override
	public IfFalseArray copy() {
		return new IfFalseArray(parent, idx);
	}
}
