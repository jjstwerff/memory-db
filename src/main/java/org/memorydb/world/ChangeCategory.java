 ...
 */
public class ChangeCategory extends Category implements ChangeInterface {
	public ChangeCategory(Store store) {
		super(store, store.allocate(Category.RECORD_SIZE));
		setType(null);
		setName(null);
		store.setInt(rec, 9, 0); // ARRAY specials
	...
	public void moveSpecials(ChangeCategory other) {
	...
	/* package private */ void parseFields(Parser parser) {
		...
		if (parser.hasSub("dependency")) {
			try (DependencyArray sub = new DependencyArray(this, -1)) {
				while (parser.getSub()) {
					sub.add();
					sub.parse(parser);
				}
		...
		if (parser.hasSub("effect")) {
			try (EffectArray sub = new EffectArray(this, -1)) {
				while (parser.getSub()) {
					sub.add();
					sub.parse(parser);
				}
