...
/**
 * Automatically generated record class for default_specials
 */

@RecordData(name = "Specials")
public class Default_specialsArray implements MemoryRecord, ChangeInterface, Iterable<Default_specialsArray> {
	...
	/* package private */ Default_specialsArray(Category parent, int idx) {
		this.store = parent.store();
		this.parent = parent;
		this.idx = idx;
		if (parent.rec() != 0) {
			this.alloc = store.getInt(parent.rec(), 9);
			if (alloc != 0) {
				up(parent);
	...
	/* package private */ Default_specialsArray(Default_specialsArray other, int idx) {
	...
	/* package private */ Default_specialsArray(Store store, int rec, int idx) {
		this.store = store;
		this.alloc = rec;
		this.idx = idx;
		this.parent = up();
	...
	@Override
	public Default_specialsArray copy(int newRec) {
		assert store.validate(newRec);
		return new Default_specialsArray(store, newRec, -1);
	}
	...
	@Override
	public Default_specialsArray add() {
		if (parent.rec() == 0)
			return this;
		if (alloc == 0) {
		...
		store.setInt(parent.rec(), 9, alloc);
		size++;
		store.setInt(alloc, 4, size);
		Default_specialsArray res = new Default_specialsArray(parent, size - 1);
		res.setSpecial(null);
		res.setKnown(false);
		res.setLevel((byte) 0);
		return res;
	...
	@Override
	public Iterator<Default_specialsArray> iterator() {
		return new Iterator<>() {
			int element = -1;
			...
			@Override
			public Default_specialsArray next() {
				if (alloc == 0 || element > size)
					throw new NoSuchElementException();
				element++;
				return new Default_specialsArray(Default_specialsArray.this, element);
			}
	...
	public String toString() {
		...
		if (idx == -1)
			for (Default_specialsArray a : this) {
				a.output(write, 4);
			}
		else
			output(write, 4);
	...
	public void parse(Parser parser) {
		...
		if (parser.hasField("special")) {
			parser.getRelation("special", (recNr, idx) -> {
				...
				Default_specialsArray old = new Default_specialsArray(store, recNr, idx);
	...
	@Override
	public Default_specialsArray index(int idx) {
		return idx < 0 || idx >= size ? null : new Default_specialsArray(parent, idx);
	}

	@Override
	public Default_specialsArray start() {
		return new Default_specialsArray(parent, 0);
	}

	@Override
	public Default_specialsArray next() {
		return idx + 1 >= size ? null : new Default_specialsArray(parent, idx + 1);
	}
	...
	@Override
	public Default_specialsArray copy() {
		return new Default_specialsArray(parent, idx);
	}
}
